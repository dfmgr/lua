...
# @@File             :  install.sh
# @@Description      :  Install configurations for lua
...
__run_pre_install() {
  ...
  __download_file "https://github.com/DhavalKapil/luaver/raw/master/luaver" "$HOME/.local/bin/luaver"
  [ -f "$HOME/.local/bin/luaver" ] && chmod 755 "$HOME/.local/bin/luaver"
  return $getRunStatus
}
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
